...
3. Then, you will run the docker container: docker-compose up -d

4. The server will be running on 8088. Send the next request to http://localhost:8088/api/connector-access-token (from POSTMAN, curl...).
...
5. This request will be returning a JSON Object called "ct". This is the capability token that allows the requester to access the resource and make an action.
...
6. Send another request to http://localhost:8080/api/access-with-token with a JSON body like this, changing the "ct" for the one that returned the last request and the "sar" for the access that the token allows you to do. 
	{
...
7. If everything is OK, you will be getting a success message. If there's any error, you get an error message as well.
